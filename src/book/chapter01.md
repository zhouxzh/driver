...
传统半导体如锗（Ge，约 0.66 eV）、硅（Si，约 1.12 eV）和砷化镓（GaAs，约 1.43 eV）的带隙通常在 2 eV 以下，而宽禁带材料的带隙在 2 eV 以上。由于宽禁带材料的带隙更大，因此它们可以在更高的电压、温度和频率下运行。宽禁带半导体用于各种应用，例如太阳能逆变器、电机驱动、电动汽车和电源。
一些宽禁带半导体的例子包括：碳化硅（SiC）、氮化镓（GaN）、二氧化钛（TiO₂）、氮化铝（AlN）、金刚石和氧化锌（ZnO）。这些材料的带隙比传统半导体更大，因此它们可以在更高的电压、温度和频率下运行。
...
| Baliga 优值 (BFOM) | 1       | 14.7   | 846    | 317    | 3334    | 24660   |

SiC（碳化硅）是一种由硅（Si）和碳（C）组成的化合物半导体材料。上表显示 SiC 具有优异的临界击穿场强和带隙，分别约为 Si 的 10 倍和 3 倍。因此，作为制作功率半导体器件的材料，SiC 可以超越 Si。SiC 具有多种多晶型（晶体多态性），每种多晶型都表现出不同的物理特性。对于功率器件，4H-SiC 被认为是理想的选择。
...
SiC 的临界击穿场强高于 Si，因此可以在相同耐压下使用更薄、掺杂浓度更高的漂移层，从而制造高击穿电压、低单位面积导通电阻的功率器件。在相同击穿电压下，SiC 漂移层的单位面积电阻理论上可降低到 Si 的约 1/300。使用 SiC 可以实现同时具有“高击穿电压”“低导通电阻”和“高开关速度”的多数载流子器件（肖特基势垒二极管和 MOSFET）。此外，SiC 的带隙约为 Si 的 3 倍，可在更高温度下工作；不过受封装耐热可靠性的限制，目前器件额定工作温度通常为 150 °C 至 175 °C，随着封装技术的进步，未来可实现 200 °C 以上的额定温度。
...
不同厂商对 SiC MOSFET 与模块的命名规则差异较大，理解命名规则有助于快速识别器件的额定电压、额定电流与导通电阻。表 1 给出了常见厂商的命名方式。

### 表1

| 制造商 | SiC MOSFET 模块命名方式 | 命名含义 | 命名规则 |
| :-----: | :-----: | :-----: | :-----: |
| Cree/Wolfspeed | C3M0120065K | 工业级第三代，导通电阻 120 mΩ、额定电压 650 V 的 SiC MOSFET | C：工业级；3：第三代；120：导通电阻；65：额定电压 |
| 意法半导体 | SCT20N120 | 额定电压 1200 V、额定电流 20 A、导通电阻 189 mΩ、TO-247（HiP247）封装的 SiC MOSFET | SCT：意法半导体 SiC MOSFET；20：额定电流；120：额定电压 |
| 三菱电机 | BM080N120K | 额定电压 1200 V、导通电阻 88 mΩ、电流 36 A、TO-247-4 封装的 N 系列 SiC MOSFET 模块 | BM：SiC MOSFET 模块；080：导通电阻；N：系列；120：额定电压 |
| 日立 | MSM800FS33ALT | 额定电压 3300 V、额定电流 800 A 的 SiC MOSFET | M：模型；S：SiC；M：2 合 1；800：额定电流；FS：封装类型；6：额定电压；ALT：芯片类型 |
| 英飞凌 | IMW120R45M1 | 导通电阻 45 mΩ、额定电压 1200 V、TO-247-9 封装的 M1 系列 SiC MOSFET | I：英飞凌；M：MOSFET；W：封装；120：额定电压；R：标识分隔；45：导通电阻；M1：SiC 第一代系列 |




### 表2
下表为意法半导体生产的 SiC MOSFET 模块系列分类：
| 参数     | Gen2                            | Gen3                        | Gen3               | Gen1           | Gen2                                | Gen3                    | Gen1                         |
| -------- | ------------------------------- | --------------------------- | ------------------ | -------------- | ----------------------------------- | ----------------------- | ---------------------------- |
| 击穿电压 | 650V                            | 650V                        | 700/900V           | 1200V          | 1200V                               | 1200V                   | 1700V                        |
| 导通电阻 | 18-55mΩ                         | 14-55mΩ                     | 11mΩ               | 52-520mΩ       | 21-75mΩ                             | 17-70mΩ                 | 65-700mΩ                     |
| 主要应用 | OBC&DC-DC、可再生能源、工业驱动 | OBC&DC-DC、太阳能、电源供应 | DC-DC、可再生能源 | HVAC、电源供应 | OBC&DC-DC、逆变器、充电桩、工业驱动 | 逆变器、DC-DC、电源供应 | DC-DC、电源供应、可再生能源 |




### 表3
下表汇总了日立、ROHM、ONSEMI 与英飞凌等厂商的典型 SiC 器件与模块参数：
| 厂商     | 系列                         | 分类               | 型号               | VDSS/VCES | ID/IC   | RON/mΩ    | 状态     |
| -------- | ---------------------------- | ------------------ | ------------------ | --------- | ------- | --------- | -------- |
| 日立     | 全SiC                        | 2in1               | MSM600GS33ALT      | 3300      | 600     |           | 开发中   |
| 日立     | 全SiC                        | 2in1               | MSM800GS33ALT      | 3300      | 800     |           | 开发中   |
| 日立     | 全SiC                        | 2in1               | MSM900GS17CLT      | 1700      | 900     |           | 开发中   |
...
| 日立     | 混合SiC                      | 1in1               | MBN1800F33F-C3     | 3300      | 1800    |           | 批量生产 |
...
随着技术的不断发展，碳化硅衬底的尺寸逐渐扩大，成本也会逐渐降低。例如，8 英寸 SiC 衬底相比 6 英寸 SiC 衬底在成本降低上具有明显优势。目前 Wolfspeed 率先实现 8 英寸 SiC 衬底量产，而大多数国际企业则将 8 英寸 SiC 衬底的量产节点定在 2023 年左右。